...
os.makedirs(BASE)
for d in ['src/navigation','src/screens','src/components','src/types','src/services/data','src/services/agents','src/store']: os.makedirs(os.path.join(BASE,d))
...
w('app.json','''{"expo":{"name":"TradePulse","slug":"tradepulse","version":"1.0.0","orientation":"portrait","icon":"./assets/icon.png","userInterfaceStyle":"light","splash":{"image":"./assets/splash.png","resizeMode":"contain","backgroundColor":"#1a1a2e"},"assetBundlePatterns":["**/*"],"ios":{"supportsTablet":true,"bundleIdentifier":"com.tradepulse.app"},"android":{"adaptiveIcon":{"foregroundImage":"./assets/adaptive-icon.png","backgroundColor":"#1a1a2e"},"package":"com.tradepulse.app"}}}''')
...
w('src/services/agents/macroAgent.ts','''import { scrapeEconomicCalendar } from '../data/forexFactory'; import { scoreMacro } from '../scoringEngine';
export async function runMacroAgent() { console.log('[Macro] Analyzing...'); const events = await scrapeEconomicCalendar();
  // Macro data from public financial data (no government sources)
  // In production, scrape from Yahoo Finance bond pages or investopedia
  const macroData = {
    treasuryYield10Y: 4.25,  // from Yahoo Finance / public sources
    treasuryYield2Y: 4.50,
    yieldCurveSpread: -0.25,
    fedFundsRate: 5.25,    // from public financial news
    realInterestRate: 2.0,
    upcomingEvents: events,
    lastCPI: 3.1,          // from public financial news
    lastJobsReport: 175,
  };
  return { score: scoreMacro(macroData), data: macroData, sources: ['ForexFactory', 'Public Financial Data'] }; }
''')
...
{expanded && <View style={styles.factorDetails}>{score.details.map((d: string, i: number) => <View key={i} style={styles.detailRow}><Ionicons name="checkmark-circle" size={14} color="#2ecc71" /><Text style={styles.detailText}>{d}</Text></View>)}
<View style={styles.rawDataBox}><Text style={styles.rawDataTitle}>Raw Data</Text>{Object.entries(score.rawData).map(([k, v]) => <Text key={k} style={styles.rawDataItem}>{k}: {v}</Text>)}</View></View>}
</View>); }
...
<View style={styles.section}><Text style={styles.sectionTitle}>Data Sources</Text><SourceItem icon="trending-up" name="Yahoo Finance" desc="Stock prices, fundamentals" /><SourceItem icon="calendar" name="ForexFactory" desc="Economic calendar" /><SourceItem icon="newspaper" name="News Headlines" desc="Simulated sentiment analysis" /></View>
...
print(f'''
========================================
TradePulse NO-API-KEY version created!
========================================
...
- News: Simulated headlines (no API)
- Macro: Public financial data (no government sources)
...
Search NVDA and watch the 4-agent swarm!
''')
