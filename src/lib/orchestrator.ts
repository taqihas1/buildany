...
`;
      ...
      // ─── ESSENTIAL FILES: Create layout, globals.css, tailwind config if missing ───
      const hasLayout = allGeneratedPaths.has('src/app/layout.tsx') || allGeneratedPaths.has('app/layout.tsx');
      const hasGlobals = allGeneratedPaths.has('src/app/globals.css') || allGeneratedPaths.has('app/globals.css');
      const hasTailwind = allGeneratedPaths.has('tailwind.config.js') || allGeneratedPaths.has('tailwind.config.ts');
...
export const metadata: Metadata = {
  title: "BuildAny App",
  description: "Generated by BuildAny",
};
...
`;
        const fullPath = path.join(projectDir, layoutPath);
        await fs.mkdir(path.dirname(fullPath), { recursive: true });
        await fs.writeFile(fullPath, layoutContent, 'utf-8');
        try {
          await db.insert(projectFiles).values({
            id: crypto.randomUUID(),
            projectId: this.state.projectId,
        ...
        allGeneratedPaths.add(layoutPath);
        console.log(`[Kelly] Created fallback ${layoutPath}`);
      }
...
`;
        const fullPath = path.join(projectDir, globalsPath);
        await fs.mkdir(path.dirname(fullPath), { recursive: true });
        await fs.writeFile(fullPath, globalsContent, 'utf-8');
        try {
          await db.insert(projectFiles).values({
            id: crypto.randomUUID(),
            projectId: this.state.projectId,
        ...
        allGeneratedPaths.add(globalsPath);
        console.log(`[Kelly] Created fallback ${globalsPath}`);
      }
  ...
  theme: {
    extend: {
      colors: {
        ...
        accent: { 50: '#f0fdfa', 500: '#14b8a6', 900: '#134e4a' },
      },
      animation: {
...
`;
        const fullPath = path.join(projectDir, tailwindPath);
        await fs.mkdir(path.dirname(fullPath), { recursive: true });
        await fs.writeFile(fullPath, tailwindContent, 'utf-8');
        try {
          await db.insert(projectFiles).values({
            id: crypto.randomUUID(),
            projectId: this.state.projectId,
        ...
        allGeneratedPaths.add(tailwindPath);
        console.log(`[Kelly] Created fallback ${tailwindPath}`);
      }
