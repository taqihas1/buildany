...
export async function POST(req: NextRequest) {
  try {
    ...
    // Clean up any nested out directories or cache from previous builds
    try { execSync("find out -mindepth 1 -maxdepth 1 -name out -type d -exec rm -rf {} +", { cwd: outDir }); } catch {}
    try { execSync("rm -rf .wrangler", { cwd: outDir }); } catch {}
