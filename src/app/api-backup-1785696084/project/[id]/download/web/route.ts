...
export async function GET(
  ...
  try {
    ...
    // Separate web files from mobile files
    const webFiles = files.filter(f => 
      !f.path.startsWith('src/screens/') && 
      !f.path.includes('react-native') &&
      !f.path.includes('expo')
    );

    if (webFiles.length === 0) {
      return NextResponse.json({ error: 'No web files found. Generate web project first.' }, { status: 404 });
    }
    ...
    return new NextResponse(Buffer.from(zipContent), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${project.name}-web.zip"`,
      },
    });
  } catch (error: any) {
