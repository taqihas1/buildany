...
# 1. Create adminStore.ts
mkdir -p src/store
cat << 'AEOF' > src/store/adminStore.ts
...
# 2. Create getLocationZip.ts
mkdir -p src/utils
cat << 'ZEOF' > src/utils/getLocationZip.ts
...
# 3. Create clean SplashScreen.tsx
cat << 'SEOF' > src/components/SplashScreen.tsx
...
# 4. Create AdminScreen.tsx
mkdir -p src/screens
cat << 'AEND' > src/screens/AdminScreen.tsx
...
# 5. Update AppNavigator.tsx to add Admin route
python3 << 'NPYEOF'
import re
...
# Add AdminScreen import if not present
...
# Add Admin to RootStackParamList if not present
...
# Add Admin Screen to Stack.Navigator if not present
...
# 6. Add Admin Panel link to SettingsScreen
python3 << 'SPYEOF'
import re
...
if 'useNavigation' not in content:
    ...
    # May already have useNavigation

# Add navigation hook and type if not present
if 'const navigation' not in content:
    # Find "export default function SettingsScreen() {" and add navigation after
    content = content.replace(
        "export default function SettingsScreen() {",
        "type NavigationProp = NativeStackNavigationProp<RootStackParamList>;\n\nexport default function SettingsScreen() {\n  const navigation = useNavigation<NavigationProp>();"
    )

# Add Admin Panel section before About section if not present
if 'Admin Panel' not in content:
    # Find the About comment and insert before it
    content = content.replace(
        "{/* About */}",
...
# 7. Add dealerAddress to carApi.ts if not already present
echo "Checking carApi.ts for dealerAddress..."
grep -n "dealerAddress" src/services/carApi.ts | head -5 || echo "dealerAddress not found, will add..."

# Check if dealerAddress is in listing generation
...
# Add dealerAddress to listing object
...
echo ""
echo "All files created! Now pushing to GitHub..."
git add -A
git commit -m "feat: add admin panel, clean splash screen, dealer addresses, location zip utils" || echo "Nothing to commit"
