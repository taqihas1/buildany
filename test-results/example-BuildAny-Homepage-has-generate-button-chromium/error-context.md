...
- Name: example.spec.ts >> BuildAny Homepage >> has generate button
- Location: e2e/example.spec.ts:15:7
...
Locator: locator('button:has-text("Generate"), button:has-text("Build")').first()
Expected: visible
Error: element(s) not found

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('button:has-text("Generate"), button:has-text("Build")').first()
